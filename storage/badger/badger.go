...
func (db *badgerDB) Init() (err error) {
	...
	if err != nil {
		return errors.Errorf("couldn't fetch database from %v | %v", db.config.Dir, err)
	}
	return
}
...
func (db *badgerDB) Get(key string) (result []byte, err error) {
	err = db.db.View(func(txn *v3Badger.Txn) error {
		...
		if err != nil {
			return errors.Errorf("couldn't fetch data by key %s | %v", key, err)
		}
...
func (db *badgerDB) Set(key string, value []byte) error {
	return db.db.Update(func(txn *v3Badger.Txn) error {
		if err := txn.Set([]byte(key), value); err != nil {
			return errors.Errorf("couldn't set data key %s value %s | %v", key, value, err)
		}
		return nil
	})
}
	...
	return db.db.Update(
		func(txn *v3Badger.Txn) error {
			if err := txn.Delete([]byte(key)); err != nil {
				return errors.Errorf("couldn't delete data by key %s | %v", key, err)
			}
			return nil
		})
}
...
func (db *badgerDB) Close() error {
	if err := db.db.Close(); err != nil {
		return errors.Errorf("couldn't close database | %v", err)
	}
	return nil
}
